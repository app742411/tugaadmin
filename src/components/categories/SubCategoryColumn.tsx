...
import { CategoryItem, SkillServiceItem } from "@/types/category.types";
import CategoryCard, { resolveImage } from "./CategoryCard";

interface SubCategoryColumnProps {
  selectedCategory: CategoryItem | null;
  skillServices: SkillServiceItem[];
  isLoading: boolean;
  error: string | null;
  selectedId: string | null;
  onSelect: (skill: SkillServiceItem) => void;
  onAdd: () => void;
  onEdit: (skill: SkillServiceItem) => void;
...
const SubCategoryColumn: React.FC<SubCategoryColumnProps> = ({
  selectedCategory,
  skillServices,
  isLoading,
  error,
  ...
  const filtered = skillServices.filter((s) =>
    s.name.toLowerCase().includes(search.toLowerCase())
  );

  // ── Idle state when no category is selected ──────────────────────────────────
  if (!selectedCategory) {
    return (
      <div className="flex flex-col h-full min-h-0">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
          ...
          <p className="text-sm font-semibold text-gray-500 dark:text-gray-400">Select a Category</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
            Skill services will load when you pick a category
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex-shrink-0 p-4 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 z-10">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search skill services..."
            className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/80 text-gray-800 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-brand-500/30 focus:border-brand-400 transition-colors"
          />
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 text-center px-4">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {search ? "No results found" : "No skill services yet"}
            </p>
            {!search && (
              <button
                onClick={onAdd}
